...
def generate_models(output, overwrite):
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "pip", "show", "flask-sqlacodegen"],
            capture_output=True,
            check=True,
        )
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "flask-sqlacodegen"], check=True
        )
    ...
    try:
        cmd = [
            ...
            str(output_path),
            "--tables",
            "users,books,samples,borrows,reservations,notifications,logs",
            db_url,
        ]
